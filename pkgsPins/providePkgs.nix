...
let
  makePkgs =
    pkgsInput: system: import pkgsInput ({ inherit system; } // config.pkgsPins.nixpkgsConfig);
  ...
  mkPins =
    ...
    let
      ...
      arbitraryPkgs = lib.optionalAttrs (!(inputs ? nixpkgs) && pinNames != [ ]) {
        pkgs = lib.mkDefault (makePkgs inputs.${"nixpkgs-${builtins.head pinNames}"} system);
      };
    in
